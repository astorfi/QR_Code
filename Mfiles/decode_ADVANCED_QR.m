...
%% Extracting bit stream(extracting the raw data from QR matrix)
str = bit_extract_Fn( Image_demask );       % Extract bit-stream related to QR-Matrix according to the bit input pattern of QR-code standard
 if module>21
     str = str(1,1:length(str)-7);    %in version 2 to 6 we always have 7 reminder bits in the end of stream. We eliminate them and then the length of string is devisible by 8.
 end
...
Bin_Str=dec_to_bin_Fn(Data_decode);     % Extract exact binary matrix of decimal sequence( The command str2num(dec2bin(...)) does not work here because for example we need the 65 as 01000001 not 1000001)
str=Bin_Str;        % Substitude binary sequence for data extraction

%% Message Extraction
Mode=Mode_Extr_Fn(str(1,1:4));      % Realizing the Mode(Alphaneumeric, BYTE, ...)
Char_Size=Char_Num_Fn(str,Mode,Error_Recov_Type,module);    % Calculating the length of message
Data_Related_stream=Data_Str_Ext_Fn(str,Char_Size,Mode,Error_Recov_Type,module);        % Extracting bits related to message(not Error correction codewords or ets)
Num_get=DecNum_Get_Fn(Data_Related_stream,Mode);      % Extracting the decimal numbers related to Data
message=Char_Get_Fn(Num_get,Char_Size,Mode);        % Transforming decimal numbers to characters
